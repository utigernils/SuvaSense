...
function App() {
  const [deviceState, setDeviceState] = useState<DeviceState>(DeviceState.BOOTLOADER)
  ...
  const handleSelftest = useCallback((sensor: string) => {
    const selftestMap: Record<string, { id: string; result: () => void }> = {
      bme680: {
        id: crypto.randomUUID(),
        result: () => setSelftest((prev) => ({ ...prev, bme680: mockSelftestBME680 })),
      },
      mpu6050: {
        id: crypto.randomUUID(),
        result: () => setSelftest((prev) => ({ ...prev, mpu6050: mockSelftestMPU6050 })),
      },
      veml7700: {
        id: crypto.randomUUID(),
        result: () => setSelftest((prev) => ({ ...prev, veml7700: mockSelftestVEML7700 })),
      },
      esp32: {
        id: crypto.randomUUID(),
        result: () => setSelftest((prev) => ({ ...prev, esp32: mockSelftestESP32 })),
      },
      led: {
        id: crypto.randomUUID(),
        result: () => setSelftest((prev) => ({ ...prev, led: mockSelftestLED })),
      },
    }
    ...
    setTimeout(() => {
      setSerialLogs((prev) => [
        ...prev,
        {
          ...
          timestamp: Date.now(),
          direction: "rx",
          raw: JSON.stringify({ type: "log", level: "info", message: `Selftest triggered for: ${sensor}` }),
          parsed: { type: "log", level: "info", message: `Selftest triggered for: ${sensor}` },
        },
      ])
      entry.result()
    }, 500)
      ...
      setTimeout(() => {
        setSerialLogs((prev) => [
          ...prev,
          {
            ...
            parsed: { type: "log", level: "info", message: "Streaming stopped" },
          },
        ])
      }, 300)
    }
  ...
  const handleSettingChange = useCallback(
    (section: keyof DeviceSettings, key: string, value: string | number | boolean) => {
      ...
      setTimeout(() => {
        const responseValue = target.includes("password") ? "***" : String(value)
        setSerialLogs((prev) => [
          ...prev,
          {
            id: crypto.randomUUID(),
            timestamp: Date.now(),
            direction: "rx",
            raw: JSON.stringify({ type: "response", action: "set", target, value: responseValue }),
            parsed: { type: "response", action: "set", target, value: responseValue },
          },
        ])
      }, 200)
    },
    ...
    setTimeout(() => {
      setSerialLogs((prev) => [
        ...prev,
        {
          ...
          parsed: { type: "log", level: "info", message: "Factory reset complete. Rebooting..." },
        },
      ])
    }, 500)
  }, [])
